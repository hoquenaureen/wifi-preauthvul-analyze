...
//Fuzzer variables to prevent ap messages from being sent
//Each variable is used in various hostap source files to prevent the corresponding AP functions (such as sending auth commits) from sending messages to the station
//
//
//fuzzer_skip_4way prevents the 4 way handshake from being sent (Doesn't completely work rn)
//fuzzer_skip_probe_response prevents probe responses from being sent from AP to station
//fuzzer_skip_auth_commit prevents the AP's auth commit message from being sent to station
//fuzzer_skip_auth_confirm prevents the AP's auth confirm message from being sent to station
//fuzzer_skip 
extern int fuzzer_skip_4way;
extern int fuzzer_skip_msg1;
extern int fuzzer_skip_msg3;
extern int fuzzer_skip_probe_response;
extern int fuzzer_skip_auth_commit;
extern int fuzzer_skip_auth_confirm;
extern int fuzzer_skip_assoc_resp;
...
//Function to process the 4way handshake boolean
//The boolean is specified in the Python3 Framework Test file as SKIP_4WAY 1
...
//Function to process the probe response boolean
//The boolean is specified in the Python3 Framework Test file as SKIP_PROBE_RESPONSE 1
static int hostapd_ctrl_skip_probe_response(struct hostapd_data *hapd, const char *cmd)
{
	fuzzer_skip_probe_response = atoi(cmd);
	wpa_printf(MSG_DEBUG, "Setting skip_probe_response to %d", fuzzer_skip_probe_response);
	return 0;
}


//Function to process the auth commit boolean
//The boolean is specified in the Python3 Framework Test file as SKIP_AUTH_COMMIT 1
static int hostapd_ctrl_skip_auth_commit(struct hostapd_data *hapd, const char *cmd)
{
	fuzzer_skip_auth_commit = atoi(cmd);
	wpa_printf(MSG_DEBUG, "Setting skip_auth_commit to %d", fuzzer_skip_auth_commit);
	return 0;
}



//Function to process the auth confirm boolean
//The boolean is specified in the Python3 Framework Test file as SKIP_AUTH_CONFIRM 1
static int hostapd_ctrl_skip_auth_confirm(struct hostapd_data *hapd, const char *cmd)
{
	fuzzer_skip_auth_confirm = atoi(cmd);
	wpa_printf(MSG_DEBUG, "Setting skip_auth_confirm to %d", fuzzer_skip_auth_confirm);
	return 0;
}


//Function to process the association response boolean
//The boolean is specified in the Python3 Framework Test file as SKIP_ASSOC_RESP 1
static int hostapd_ctrl_skip_assoc_resp(struct hostapd_data *hapd, const char *cmd)
{
	fuzzer_skip_assoc_resp = atoi(cmd);
	wpa_printf(MSG_DEBUG, "Setting skip_assoc_resp to %d", fuzzer_skip_assoc_resp);
	return 0;
}


static int hostapd_ctrl_skip_msg1(struct hostapd_data *hapd, const char *cmd)
{
	fuzzer_skip_msg1 = atoi(cmd);
	wpa_printf(MSG_DEBUG, "Setting skip_msg1 to %d", fuzzer_skip_msg1);
	return 0;
}


static int hostapd_ctrl_skip_msg3(struct hostapd_data *hapd, const char *cmd)
{
	fuzzer_skip_msg3 = atoi(cmd);
	wpa_printf(MSG_DEBUG, "Setting skip_msg3 to %d", fuzzer_skip_msg3);
	return 0;
}
					      ...
					      socklen_t fromlen)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		//Call the skip 4way function to process the boolean
		if (hostapd_ctrl_skip_4way(hapd, buf + 10) < 0 )
			reply_len = -1;
	} else if (os_strncmp(buf, "SKIP_PROBE_RESPONSE ", 20) == 0) {
		//Call the skip probe response function to process the boolean
		if (hostapd_ctrl_skip_probe_response(hapd, buf + 20) < 0 )
			reply_len = -1;
	} else if (os_strncmp(buf, "SKIP_AUTH_COMMIT ", 17) == 0) {
		//Call the skip auth commit function to process the boolean
		if (hostapd_ctrl_skip_auth_commit(hapd, buf + 17) < 0 )
			reply_len = -1;
	} else if (os_strncmp(buf, "SKIP_AUTH_CONFIRM ", 18) == 0) {
		//Call the skip auth confirm function to process the boolean
		if (hostapd_ctrl_skip_auth_confirm(hapd, buf + 18) < 0 )
			reply_len = -1;
	} else if (os_strncmp(buf, "SKIP_ASSOC_RESP ", 16) == 0) {
		//Call the skip association response function to process the boolean
		if (hostapd_ctrl_skip_assoc_resp(hapd, buf + 16) < 0 )
			reply_len = -1;
	} else if (os_strncmp(buf, "SKIP_MSG1 ", 10) == 0) {
		//Call the skip 4way function to process the boolean
		if (hostapd_ctrl_skip_msg1(hapd, buf + 10) < 0 )
			reply_len = -1;
